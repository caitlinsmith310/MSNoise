...
def main(mov_stack=None, dttname="M", components='ZZ', filterid=1,
         pairs=[], showALL=False, show=False, outfile=None ,scale=False):
    db = connect()
    ...
    for i, mov_stack in enumerate(mov_stacks):
        ...
        if 'alldf' in locals():
            ...
            tmp2.plot(label='mean', color='orange')

            tmp4 = allbut[dttname].resample('D').mean()+allbut[dttname].resample('D').std()
            tmp4.plot(label='meanstd-',color='red')
     

            tmp5 = allbut[dttname].resample('D').mean()-allbut[dttname].resample('D').std()
            tmp5.plot(label='meanstd+',color='red')
            ...
            if first_plot == 1:
                ...
                if mov_stack == 1:
                    ...
                    plt.title('%i Days Average' % mov_stack)
                first_plot = False
            else:
                plt.xlim(left, right)
                plt.title('%i Days Average' % mov_stack)
            ...
            plt.grid(True)
            #if scale=True
            #plt.ylim(min(tmp5),max(tmp4))# MY ADJUSTMENT!!!!!
            plt.ylim(lim1,lim2)# MY ADJUSTMENT!!!!!
            del alldf
    if outfile:
